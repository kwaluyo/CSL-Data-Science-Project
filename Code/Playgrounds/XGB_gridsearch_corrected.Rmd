...
# Set up the paralleling maparmaters
N_CLUSTER = 15
N_CHUNKS = N_CLUSTER
...
```{r}
# Ok so here the new idea is as follow, 
...
# Kartika open the object "indexes" and look what is inside so you'll understand what happens in the parallel loop
...
```{r}
# a list of all trained genes

#files <- list.files(paste0(dir,"/trained/"))
#trained_gene = c()
#for(z in 1:length(files)){
#  trained_gene = c(trained_gene, unlist(strsplit(files[z], split='.RData', fixed=TRUE))[1])
#}
```
...
#training_pred <- data.frame(matrix(NA,nrow=nrow(training),ncol=end))
...
#  file <- files[z]
#  load(paste0(dir,"/trained2/",file))
#  colnames(training_pred)[z] <- gene_name
#  training_pred[,z] <- predict(model, as.matrix(training))
#  if (z%%1000==0){print(z)}
#}
...
# Calculate correaltions with the training target set
...
# Observed the median correaltion
median(correlations)
...
#test_pred <- data.frame(matrix(NA,nrow=nrow(test),ncol=end))
#files <- list.files(paste0(dir,"/trained/"))
...
#  
#  file <- files[z]
##  load(paste0(dir,"/trained/",file))
#  colnames(test_pred)[z] <- gene_name
#  test_pred[,z] <- predict(model, as.matrix(test))
#  
#}
...
# Calculate correaltions with the test target set
correlations <- vector(length = end)
...
#Please continue this script with the analyses decided before
...
# CAREFUL HERE!!!!
#lung$genes$correlations <- correlations
#lung$samples
#lung$cmp
