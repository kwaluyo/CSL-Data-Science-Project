...
This task involves the exploration of correlation between gene expressions in Whole blood with the expression of the same genes in other tissues. Additionally, the correlation of each gene found in Whole Blood with each tissue is also explored. Finally, the tissues are ranked on its similarity with whole blood on multiple meassures.
...
```{r echo=FALSE , fig.cap = "Mean Correlation of a Tissue and Whole Blood"}
...
```
The above plot captures the mean correlation between a particular tissue with Whole Blood, based on their shared genes.  
...
Since this plot considers absolute correlation of each gene in Whole Blood with the same gene found in all other tissues, the x axis has a lower bound at zero. The majority of genes in other tissues have a correlation of about 0.10, but a very small number of them have a strong correlation.
