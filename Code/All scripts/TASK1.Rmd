...
```{r echo=FALSE}
# Compute overlapping donor proportion
...
```{r echo=FALSE}
# Compute overlapping gene proportions 
row = 1
for (i in tissue_list) {
  col = 1
  for (j in tissue_list) {
...
pheatmap(tissue_donor_proportion, main="Heatmap of Donor Overlap Proportions in All Pairs of Tissues", fontsize = 15)
```
...
```{r echo=FALSE}
# Compute Proportion of Overlapping Donors of Whole Blood with all other Tissues
...
```{r echo=FALSE}
# Compute Proportion of Overlapping Donors of all Tissues with Whole Blood
...
```{r figs4, echo=FALSE, fig.height=7, fig.width=26, fig.cap="\\label{fig:figs4}Bar plot representing the proportion of overlapping donors of whole blood with other tissues"}
ggplot(data=whole_blood_row_donor, aes(x=reorder(subtissue,-proportion), y=proportion, fill = tissue)) +
  ...
  xlab("Sub-Tissues") + ylab("Proportion") +
  theme(axis.text.x = element_text(hjust=1, angle = 55, size=12),
  ...
  geom_text(aes(label = round(proportion, 3)), size = 4, angle=90, hjust=-0.1)
...
pheatmap(shared_gene_proportion, main="Heatmap of Overlapping Gene Proportions in All Pairs of Tissues", fontsize = 15)
```
```{r}
...
```{r figs8, fig.height=7, fig.width=25, echo=FALSE, fig.cap="\\label{fig:figs8}Bar plot representing the proportion of overlapping genes of whole blood with all other tissues"}
